...
const StageDisplayMusicos = ({ eventoIdOverride, defaultViewMode = 'musico', storageScope = 'stageDisplay', user }) => {
  ...
  const eventSongIds = useMemo(() => getSongIdsFromEvent(evento), [evento?.setlist, evento?.canciones]);
  const eventSongIdsKey = eventSongIds.join('|');
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, 'eventos', eventoId), (snap) => {
      ...
      const nextSongId = nextLiveState.activeSongId;
  ...
  useEffect(() => {
    let cancelled = false;
    if (!eventSongIds.length) {
      setEventSongs([]);
      return undefined;
    }

    const fetchEventSongs = async () => {
      const uniqueIds = [...new Set(eventSongIds)];
  ...
  const toneInfo = useMemo(() => {
    ...
    const singer = getEventSingerForSong(evento, songData.id);
    targetTone = getSingerTone(songData, singer) || targetTone;

    if (!targetTone) return '';
    return traducirAcorde(targetTone, formato, notacion);
  }, [songData, evento, formato, notacion]);
  ...
  const scaledLyricSize = `calc(${lyricSize} * ${fontScale} * var(--return-content-scale, 1))`;
  const scaledChordSize = `calc(${chordSize} * ${fontScale} * var(--return-content-scale, 1))`;
  const prepareText = nextSlide
    ...
    : nextSong || '---';
  const fitCurrentSection = !manualMode && displayedSongIsLive && currentIndex >= 0 && currentIndex < secciones.length;
  const displayedSections = fitCurrentSection
    ? [{ sec: secciones[currentIndex], idx: currentIndex }]
    : secciones.map((sec, idx) => ({ sec, idx }));
  const activeSectionContentKey = fitCurrentSection ? JSON.stringify(secciones[currentIndex]) : '';

  useFitReturnContent({
    containerRef: musicAreaRef,
    contentRef: musicContentRef,
    enabled: fitCurrentSection,
    minScale: isMobileLandscape ? 0.78 : 0.72,
    dependencies: [songId, currentIndex, activeSectionContentKey, fontScale, shouldShowChords, isSingerMode, offset, formato, notacion, isMobileLandscape]
  });
    ...
    >
      ...
      <header className="relative z-10 shrink-0 border-b border-zinc-800/80 bg-zinc-950/90 backdrop-blur-sm">
        <div className={`mx-auto flex w-full max-w-7xl items-center justify-between gap-3 px-3 sm:px-5 lg:px-8 ${isMobileLandscape ? 'gap-2 px-2 py-px' : 'py-2 lg:py-4'}`}>
          <div className={`flex min-w-0 items-center gap-2 lg:gap-4 ${isMobileLandscape ? 'gap-1.5' : ''}`}>
            <button onClick={() => navigate(`/setlist/${eventoId}`)} className={`shrink-0 rounded-full bg-zinc-800/70 text-zinc-400 transition-colors hover:bg-zinc-700 hover:text-white ${isMobileLandscape ? 'p-1' : 'p-2'}`} title="Volver">
              <ArrowLeft className={isMobileLandscape ? 'h-4 w-4' : 'h-5 w-5'} />
            </button>
            <div className="min-w-0">
              <div className={`${manualMode ? 'text-amber-400' : 'text-emerald-400'} flex items-center gap-2 font-black uppercase ${isMobileLandscape ? 'gap-1 text-[7px] leading-none tracking-[0.1em]' : 'text-[10px] tracking-[0.18em]'}`}>
                <span className={`${isMobileLandscape ? 'h-1.5 w-1.5' : 'h-2 w-2'} rounded-full ${manualMode ? 'bg-amber-400' : 'bg-emerald-400 animate-pulse'}`} />
                {manualMode ? 'Modo manual' : 'Siguiendo en vivo'}
              </div>
              <h1 className={`truncate font-black uppercase tracking-wide text-zinc-100 ${isMobileLandscape ? 'text-[15px] leading-tight' : 'text-lg sm:text-2xl lg:text-4xl'}`}>
                {displayedSongTitle}
              </h1>
            </div>
          </div>
          ...
          <div className={`${isMobileLandscape ? 'block' : 'hidden sm:block'} shrink-0 text-right`}>
            <div className={`font-mono font-black tabular-nums text-white ${isMobileLandscape ? 'text-base leading-none' : 'text-2xl lg:text-4xl'}`}>
        ...
        <div className={`mx-auto grid w-full max-w-7xl grid-cols-4 gap-2 px-3 sm:px-5 lg:px-8 ${isMobileLandscape ? 'gap-1 px-2 pb-px' : 'pb-2 lg:pb-4'}`}>
          <div className={`rounded-xl border border-zinc-800 bg-zinc-900/70 px-3 ${isMobileLandscape ? 'rounded-lg px-1.5 py-px' : 'py-2'}`}>
            <p className={`${isMobileLandscape ? 'text-[7px] leading-none tracking-[0.08em]' : 'text-[9px] tracking-widest'} font-black uppercase text-zinc-500`}>Tono</p>
            <p className={`truncate font-black text-yellow-300 ${isMobileLandscape ? 'mt-0.5 text-xs leading-none' : 'text-base lg:text-2xl'}`}>{toneInfo || '--'}</p>
          </div>
          <div className={`rounded-xl border border-zinc-800 bg-zinc-900/70 px-3 ${isMobileLandscape ? 'rounded-lg px-1.5 py-px' : 'py-2'}`}>
            <p className={`${isMobileLandscape ? 'text-[7px] leading-none tracking-[0.08em]' : 'text-[9px] tracking-widest'} font-black uppercase text-zinc-500`}>Actual</p>
            <p className={`truncate font-black text-blue-300 ${isMobileLandscape ? 'mt-0.5 text-xs leading-none' : 'text-sm lg:text-xl'}`}>{activeSectionTitle}</p>
          </div>
          <div className={`rounded-xl border border-zinc-800 bg-zinc-900/70 px-3 ${isMobileLandscape ? 'rounded-lg px-1.5 py-px' : 'py-2'}`}>
            <p className={`${isMobileLandscape ? 'text-[7px] leading-none tracking-[0.08em]' : 'text-[9px] tracking-widest'} font-black uppercase text-zinc-500`}>Siguiente</p>
            <p className={`truncate font-black text-emerald-300 ${isMobileLandscape ? 'mt-0.5 text-xs leading-none' : 'text-sm lg:text-xl'}`}>{nextSectionTitle}</p>
          </div>
          <div className={`rounded-xl border border-zinc-800 bg-zinc-900/70 px-3 py-2 ${isMobileLandscape ? 'rounded-lg px-1.5 py-px' : ''}`}>
            <p className={`${isMobileLandscape ? 'text-[7px] leading-none tracking-[0.08em]' : 'text-[9px] tracking-widest'} font-black uppercase text-zinc-500`}>Actualizado por</p>
            <p className={`truncate font-black text-zinc-300 ${isMobileLandscape ? 'mt-0.5 text-xs leading-none' : 'text-sm'}`}>{liveState?.updatedBy || 'Multimedia'}</p>
          </div>
        </div>
      ...
      {shouldShowAlert && alertPriority === 'urgente' && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[radial-gradient(circle_at_50%_35%,rgba(248,113,113,0.92),rgba(185,28,28,0.82)_45%,rgba(69,10,10,0.72)_100%)] p-8 text-center backdrop-blur-[2px]">
          ...
          <div className="relative rounded-[2rem] border border-white/20 bg-black/20 px-8 py-7 shadow-2xl">
            <h2 className="mb-4 text-4xl font-black uppercase italic text-white md:text-6xl">Urgente / Atención musicos</h2>
      ...
      {shouldShowAlert && alertPriority !== 'urgente' && (
        <div className={`fixed left-3 right-3 z-[90] mx-auto max-w-5xl rounded-3xl border px-5 py-4 text-center shadow-2xl backdrop-blur-md ${alertPriority === 'importante' ? 'top-24 border-amber-300/50 bg-amber-500/90 text-zinc-950' : 'bottom-24 border-white/10 bg-zinc-950/88 text-white'}`}>
          <p className={`text-[10px] font-black uppercase tracking-[0.24em] ${alertPriority === 'importante' ? 'text-zinc-900/70' : 'text-emerald-300'}`}>
            {alertPriority === 'importante' ? 'Atención musicos' : 'Mensaje a musicos'}
          </p>
          <p className="mt-1 text-2xl font-black leading-tight sm:text-4xl">{alertaData.text}</p>
        </div>
      )}

      <main ref={musicAreaRef} className={`relative z-0 min-h-0 flex-1 overflow-y-auto overflow-x-hidden scroll-pb-6 px-2 sm:px-5 lg:px-8 [&::-webkit-scrollbar]:hidden ${isMobileLandscape ? 'py-0.5' : 'pb-6 pt-3 lg:pb-8 lg:pt-7'}`}>
        {showLogo ? (
          <div className="flex h-full items-center justify-center text-center">
            <span className="text-2xl font-black italic text-zinc-700 sm:text-5xl">Logo Kadosh en pantalla</span>
          </div>
        ) : secciones.length > 0 ? (
          <div ref={musicContentRef} className={`mx-auto flex w-full max-w-7xl flex-col ${fitCurrentSection ? (isMobileLandscape ? 'gap-0 pb-0' : 'gap-2 pb-2') : (isMobileLandscape ? 'gap-2 pb-2' : 'gap-7 pb-8 landscape:gap-4 lg:gap-10')}`}>
            {displayedSections.map(({ sec, idx }) => {
              ...
              return (
                <section
                  key={`${sec.titulo}-${idx}`}
                  id={`section-${idx}`}
                  className={`rounded-2xl border px-3 transition-all duration-300 sm:px-6 lg:px-9 ${fitCurrentSection ? (isMobileLandscape ? 'py-1' : 'py-2 sm:py-3 lg:py-4') : (isMobileLandscape ? 'py-2' : 'py-4 sm:py-6 lg:py-8')} ${isActive ? 'border-blue-400 bg-blue-500/12 shadow-[0_0_45px_rgba(59,130,246,0.2)]' : 'border-white/0 bg-transparent opacity-85'}`}
                >
                  <div className={`flex items-center justify-between gap-3 ${fitCurrentSection ? (isMobileLandscape ? 'mb-1' : 'mb-1.5') : (isMobileLandscape ? 'mb-2' : 'mb-4')}`}>
                  ...
                  <div className={`flex flex-col ${fitCurrentSection ? (isMobileLandscape ? 'gap-1' : 'gap-1.5 sm:gap-2') : (isMobileLandscape ? 'gap-2' : isSingerMode ? 'gap-3 sm:gap-4' : 'gap-4 sm:gap-6')}`}>
                    {(sec.items?.length ? sec.items : sec.lineas.map(line => ({ type: 'lyrics', line }))).map((item, idxL) => item.type === 'cue' ? (
                      <div key={idxL} className={`mx-auto inline-flex max-w-full items-center justify-center rounded-2xl border text-center font-black uppercase tracking-wide ${fitCurrentSection && isMobileLandscape ? 'px-3 py-1' : 'px-4 py-2'} ${isSingerMode ? 'border-violet-300/50 bg-violet-500/25 text-violet-100' : 'border-violet-400/30 bg-violet-500/15 text-violet-200'}`} style={{ fontSize: `calc(${isSingerMode ? 'clamp(1rem, 3.4vmin, 2.1rem)' : 'clamp(0.82rem, 2.4vmin, 1.35rem)'} * ${fontScale} * var(--return-content-scale, 1))` }}>
                        * {item.text}
                      </div>
                    ) : item.type === 'blank' ? (
                          ...
                          <div key={idxP} className="flex items-end whitespace-nowrap">
                            {palabra.map((silaba, idxS) => (
                              <span key={idxS} className="inline-flex flex-col items-start justify-end">
                                {shouldShowChords && (
                                  <span
                                    className={`${fitCurrentSection ? (isMobileLandscape ? 'mb-0' : 'mb-0.5') : 'mb-1'} flex min-h-[1.05em] items-end font-black leading-none ${isActive ? 'text-yellow-300' : 'text-zinc-500'}`}
      ...
      <footer className={`relative z-10 shrink-0 border-t border-zinc-800 bg-zinc-950/95 px-2 shadow-[0_-10px_30px_rgba(0,0,0,0.45)] sm:px-5 lg:px-8 ${isMobileLandscape ? 'py-px' : 'py-2'}`}>
        <div className={`mx-auto flex w-full max-w-7xl gap-2 md:flex-row md:items-center md:justify-between ${isMobileLandscape ? 'gap-1.5 flex-row items-center' : 'flex-col'}`}>
          <div className={`flex min-w-0 items-center rounded-xl border border-zinc-800 bg-zinc-900/80 px-3 md:flex-1 ${isMobileLandscape ? 'gap-1.5 rounded-lg px-2 py-px' : 'gap-3 py-2'}`}>
            <div className={`flex shrink-0 items-center justify-center rounded-full border border-zinc-700 bg-zinc-800 ${isMobileLandscape ? 'h-5 w-5' : 'h-9 w-9'}`}>
              <ChevronRight className={`${isMobileLandscape ? 'h-3 w-3' : 'h-5 w-5'} text-emerald-300`} />
            </div>
            <div className="min-w-0">
              <p className={`${isMobileLandscape ? 'text-[7px] leading-none tracking-[0.08em]' : 'text-[9px] tracking-widest'} font-black uppercase text-zinc-500`}>Preparar</p>
              <p className={`truncate font-black text-zinc-200 ${isMobileLandscape ? 'mt-0.5 text-xs leading-none' : 'text-sm sm:text-xl'}`}>{prepareText}</p>
            </div>
          </div>

          <div className={`flex items-center gap-2 overflow-x-auto pb-0.5 md:justify-end [&::-webkit-scrollbar]:hidden ${isMobileLandscape ? 'shrink-0 gap-1 pb-0' : ''}`}>
            <button onClick={() => setViewMode(isSingerMode ? 'musico' : 'cantante')} className={`inline-flex shrink-0 items-center gap-2 rounded-xl border font-black uppercase ${isMobileLandscape ? 'rounded-lg px-1.5 py-1 text-[0px]' : 'px-3 py-2 text-[11px]'} ${isSingerMode ? 'border-pink-400/40 bg-pink-500/15 text-pink-200' : 'border-blue-400/40 bg-blue-500/15 text-blue-200'}`}>
              {isSingerMode ? <Mic2 size={isMobileLandscape ? 14 : 16} /> : <Guitar size={isMobileLandscape ? 14 : 16} />}
              <span className={isMobileLandscape ? 'sr-only' : ''}>{isSingerMode ? 'Cantante' : 'Musico'}</span>
            </button>
            <button onClick={() => setShowChords(prev => !prev)} className={`inline-flex shrink-0 items-center gap-2 rounded-xl border border-zinc-700 bg-zinc-900 font-black uppercase text-zinc-300 ${isMobileLandscape ? 'rounded-lg px-1.5 py-1 text-[0px]' : 'px-3 py-2 text-[11px]'}`}>
              {shouldShowChords ? <EyeOff size={isMobileLandscape ? 14 : 16} /> : <Eye size={isMobileLandscape ? 14 : 16} />}
              <span className={isMobileLandscape ? 'sr-only' : ''}>{shouldShowChords ? 'Ocultar acordes' : 'Ver acordes'}</span>
            </button>
            <button onClick={() => adjustFont(-0.08)} className={`inline-flex shrink-0 items-center justify-center rounded-xl border border-zinc-700 bg-zinc-900 text-zinc-300 ${isMobileLandscape ? 'h-6 w-6 rounded-lg' : 'h-10 w-10'}`} title="Reducir letra">
              <Minus size={isMobileLandscape ? 14 : 17} />
            </button>
            <button onClick={() => adjustFont(0.08)} className={`inline-flex shrink-0 items-center justify-center rounded-xl border border-zinc-700 bg-zinc-900 text-zinc-300 ${isMobileLandscape ? 'h-6 w-6 rounded-lg' : 'h-10 w-10'}`} title="Aumentar letra">
              <Plus size={isMobileLandscape ? 14 : 17} />
            </button>
            <button onClick={() => setShowConfig(!showConfig)} className={`inline-flex shrink-0 items-center justify-center rounded-xl border ${isMobileLandscape ? 'h-6 w-6 rounded-lg' : 'h-10 w-10'} ${showConfig ? 'border-violet-400 bg-violet-600 text-white' : 'border-zinc-700 bg-zinc-900 text-zinc-300'}`} title="Ajustes">
              <Settings2 size={isMobileLandscape ? 14 : 17} />
            </button>
            {manualMode && (
