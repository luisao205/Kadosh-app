...
import { canCreateAnnouncements, canDeleteAnnouncements, canEditAnnouncements, canProjectAnnouncements, canViewAnnouncements } from '../../utils/announcementPermissions';
...
const AnnouncementManagement = ({ user }) => {
  const allowed = canViewAnnouncements(user); const canCreate = canCreateAnnouncements(user); const canEdit = canEditAnnouncements(user); const canProject = canProjectAnnouncements(user);
  ...
  const save = async () => { if (draft.id ? !canEdit : !canCreate) return notify('No tienes permiso para guardar anuncios.', { type: 'error' }); const data = { title: draft.title.trim() || 'Anuncio', status: draft.status || 'draft', slides: draft.slides || [], updatedAt: serverTimestamp(), updatedBy: user.uid }; if (draft.id) await setDoc(doc(db,'anuncios',draft.id),data,{merge:true}); else await addDoc(collection(db,'anuncios'),{...data,createdAt:serverTimestamp(),createdBy:user.uid}); setDraft(null); notify('Anuncio guardado.',{type:'success'}); };
  const present = async (announcement, index=0, autoAdvance=false) => { if (!canProject) return notify('No tienes permiso para proyectar anuncios.', { type: 'error' }); if (!eventId) return notify('Selecciona un evento.',{type:'warning'}); await setDoc(doc(db,'eventos',eventId),buildAnnouncementProjectionPayload({announcement,slideIndex:index,user,autoAdvance}),{merge:true}); setPresenting({announcement,index,autoAdvance}); };
  ...
  return <div className="h-full overflow-y-auto p-4 sm:p-6"><div className="mx-auto max-w-7xl"><div className="mb-5 flex flex-wrap items-center gap-3"><div className="mr-auto"><h1 className="text-2xl font-black"><Megaphone className="mr-2 inline text-amber-400"/>Anuncios</h1><p className="text-sm text-zinc-500">Presentaciones reutilizables para congregacion</p></div>{canProject && <select value={eventId} onChange={e=>setEventId(e.target.value)} className="rounded-md border border-white/10 bg-zinc-900 px-3 py-2"><option value="">Evento destino</option>{events.map(e=><option key={e.id} value={e.id}>{e.titulo || e.id}</option>)}</select>}{canCreate && <button onClick={()=>setDraft(createBlankAnnouncement())} className="inline-flex items-center gap-2 rounded-md bg-violet-600 px-4 py-2 text-sm font-black"><Plus/>Crear</button>}</div>
  {canCreate && <div className="mb-5 flex gap-2 overflow-x-auto">{ANNOUNCEMENT_TEMPLATES.map(t=><button key={t.id} onClick={()=>setDraft(t.create())} className="shrink-0 rounded-md border border-white/10 bg-zinc-900 px-3 py-2 text-xs font-black">{t.name}</button>)}</div>}
  ...
  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{items.map(item=><article key={item.id} className="rounded-md border border-white/10 bg-zinc-900 p-4"><h2 className="truncate text-lg font-black">{item.title}</h2><p className="text-xs text-zinc-500">{item.slides?.length||0} diapositivas</p><div className="mt-4 flex gap-2">{canEdit && <button onClick={()=>setDraft(item)} title="Editar"><Edit3/></button>}{canProject && <button onClick={()=>present(item)} title="Presentar"><Play/></button>}{canCreate && <button onClick={()=>setDraft({...item,id:null,title:`${item.title} copia`})} title="Duplicar"><Copy/></button>}{canDeleteAnnouncements(user)&&<button onClick={async()=>{if(await confirm({title:'Eliminar anuncio',message:'Esta accion no se puede deshacer.',confirmLabel:'Eliminar',tone:'danger'}))await deleteDoc(doc(db,'anuncios',item.id));}} title="Eliminar" className="text-red-400"><Trash2/></button>}</div></article>)}</div></div></div>;
};
